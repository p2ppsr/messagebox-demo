...
class MessageBoxClient {
    ...
    constructor(options = {}) {
        ...
        const defaultHost = networkPreset === 'testnet'
   ...
   */
    ...
    async initializeConnection(overrideHost) {
        ...
        if (((_a = this.socket) === null || _a === void 0 ? void 0 : _a.connected) && !this.socketAuthenticated) {
        ...
        this.connectionInitPromise = new Promise((resolve, reject) => {
            ...
            socketAny === null || socketAny === void 0 ? void 0 : socketAny.on('authenticationSuccess', onSuccess);
            socketAny === null || socketAny === void 0 ? void 0 : socketAny.on('authenticationFailed', onFailed);
            socketAny === null || socketAny === void 0 ? void 0 : socketAny.on('disconnect', onDisconnectBeforeAuth);
   ...
   */
    ...
    async anointHost(host) {
        ...
        const spendableOutpoints = new Set(basketResult.outputs.filter(o => o.spendable).map(o => o.outpoint));
        ...
        const pushdrop = new sdk_1.PushDrop(this.walletClient, this.originator);
        ...
        try {
            ...
            if (tokensToSpend.length > 0) {
                const mergedBeef = sdk_1.Beef.fromBinary(tokensToSpend[0].beef);
                for (let i = 1; i < tokensToSpend.length; i++) {
                    mergedBeef.mergeBeef(sdk_1.Beef.fromBinary(tokensToSpend[i].beef));
                }
            ...
            if (signableTransaction === undefined) {
                if (directTx === undefined)
                    throw new Error('Anoint failed: no transaction returned');
                Logger.log('[MB CLIENT] Transaction created (no inputs to sign):', directTxid);
                const broadcaster = new sdk_1.TopicBroadcaster(['tm_messagebox'], { networkPreset: this.networkPreset });
                const result = await broadcaster.broadcast(sdk_1.Transaction.fromAtomicBEEF(directTx));
            ...
            for (let i = 0; i < tokensToSpend.length; i++) {
                ...
                const sourceTx = sdk_1.Transaction.fromBEEF(token.beef);
                const sourceSatoshis = (_b = (_a = sourceTx.outputs[token.outputIndex]) === null || _a === void 0 ? void 0 : _a.satoshis) !== null && _b !== void 0 ? _b : 1;
                const unlocker = pushdrop.unlock([1, 'messagebox advertisement'], '1', 'anyone', 'all', false, sourceSatoshis, token.lockingScript);
    ...
    async revokeHostAdvertisement(advertisementToken) {
        ...
        try {
            ...
            const sourceTx = sdk_1.Transaction.fromBEEF(advertisementToken.beef);
            const sourceSatoshis = (_b = (_a = sourceTx.outputs[advertisementToken.outputIndex]) === null || _a === void 0 ? void 0 : _a.satoshis) !== null && _b !== void 0 ? _b : 1;
            // Prepare the unlocker
            const pushdrop = new sdk_1.PushDrop(this.walletClient, this.originator);
            const unlocker = pushdrop.unlock([1, 'messagebox advertisement'], '1', 'anyone', 'all', false, sourceSatoshis, advertisementToken.lockingScript);
    ...
    async listMessages({ messageBox, host, acceptPayments }) {
        ...
        if (hosts.length === 0) {
            const advertisedHosts = await this.queryAdvertisements(await this.getIdentityKey(), undefined);
