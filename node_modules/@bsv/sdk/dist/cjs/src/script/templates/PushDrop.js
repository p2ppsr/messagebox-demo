...
const SignatureUtils_js_1 = require("./SignatureUtils.js");
/**
...
class PushDrop {
     ...
     */
    static decode(script, lockPosition = 'before') {
        let lockingPublicKey;
        let startIndex;
        if (lockPosition === 'before') {
            lockingPublicKey = index_js_2.PublicKey.fromString(index_js_2.Utils.toHex((0, utils_js_1.verifyNotNull)(script.chunks[0].data, 'script.chunks[0].data must have value')));
            startIndex = 2;
        }
        else {
            // lockPosition === 'after'
            ...
            if (script.chunks[lastChunkIndex].op !== index_js_1.OP.OP_CHECKSIG) {
                throw new Error('Expected OP_CHECKSIG at the end of the script');
            }
            lockingPublicKey = index_js_2.PublicKey.fromString(index_js_2.Utils.toHex((0, utils_js_1.verifyNotNull)(script.chunks[lastChunkIndex - 1].data, 'public key chunk data must have value')));
            startIndex = 0;
        }
        const fields = [];
    ...
    unlock(protocolID, keyID, counterparty, signOutputs = 'all', anyoneCanPay = false, sourceSatoshis, lockingScript) {
        return {
            sign: async (tx, inputIndex) => {
                ...
                const preimage = (0, SignatureUtils_js_1.formatPreimage)({
