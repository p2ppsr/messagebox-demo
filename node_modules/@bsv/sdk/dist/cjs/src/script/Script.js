...
class Script {
    ...
    static fromHex(hex) {
        ...
        const bin = (0, utils_js_1.toArray)(hex, 'hex');
    ...
    toHex() {
        ...
        const hex = BufferCtor != null
            ...
            : (0, utils_js_1.encode)(Array.from(this.rawBytesCache), 'hex');
        this.hexCache = hex;
        return hex;
    }
    /**
    ...
    findAndDelete(script) {
        ...
        const matchesChunk = (chunk) => {
            ...
            if (chunk.op === OP_js_1.default.OP_RETURN) {
            ...
            if (chunk.op < OP_js_1.default.OP_PUSHDATA1) {
                if (targetLen !== 1 + dataLen)
                    return false;
                for (let j = 0; j < dataLen; j++) {
                    if (targetBytes[1 + j] !== dataArr[j])
            ...
            if (chunk.op === OP_js_1.default.OP_PUSHDATA2) {
                if (targetLen !== 3 + dataLen)
                    return false;
                if (targetBytes[1] !== (dataLen & 0xff))
                    return false;
                if (targetBytes[2] !== ((dataLen >> 8) & 0xff))
                    return false;
                for (let j = 0; j < dataLen; j++) {
            ...
            if (chunk.op === OP_js_1.default.OP_PUSHDATA4) {
                if (targetLen !== 5 + dataLen)
                    return false;
                const size = dataLen >>> 0;
                if (targetBytes[1] !== (size & 0xff))
                    return false;
                if (targetBytes[2] !== ((size >> 8) & 0xff))
                    return false;
                if (targetBytes[3] !== ((size >> 16) & 0xff))
                    return false;
                if (targetBytes[4] !== ((size >> 24) & 0xff))
                    return false;
                for (let j = 0; j < dataLen; j++) {
    ...
    static computeSerializedLength(chunks) {
        ...
        for (const chunk of chunks) {
            ...
            else if (chunk.op === OP_js_1.default.OP_PUSHDATA1) {
                total += 1 + len;
            }
            else if (chunk.op === OP_js_1.default.OP_PUSHDATA2) {
                total += 2 + len;
            }
            else if (chunk.op === OP_js_1.default.OP_PUSHDATA4) {
                total += 4 + len;
            }
        }
        return total;
    ...
    static parseChunks(bytes) {
        ...
        while (pos < length) {
            ...
            if (op === OP_js_1.default.OP_IF ||
                op === OP_js_1.default.OP_NOTIF ||
