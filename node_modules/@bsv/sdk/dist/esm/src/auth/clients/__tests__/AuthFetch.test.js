...
function createWalletStub() {
    ...
    return {
        getPublicKey: jest.fn(async (options) => {
...
describe('AuthFetch payment handling', () => {
    test('createPaymentContext builds a complete retry context', async () => {
        ...
        expect(context.requestSummary.bodyByteLength).toBe(Utils.toArray(JSON.stringify(config.body), 'utf8').length);
    ...
    test('handlePaymentAndRetry reuses compatible contexts and adds payment header', async () => {
        ...
        const fetchSpy = jest.spyOn(authFetch, 'fetch').mockResolvedValue({ status: 200 });
        jest.spyOn(authFetch, 'logPaymentAttempt').mockImplementation(() => { });
        const createPaymentContextSpy = jest.spyOn(authFetch, 'createPaymentContext');
        const config = {
...
describe('AuthFetch certificate request handling', () => {
    ...
    test('sendCertificateRequest resolves and cleans up listener when certificates are received', async () => {
        const wallet = createWalletStub();
        const authFetch = new AuthFetch(wallet);
        let certificatesListener;
        const stopListeningForCertificatesReceived = jest.fn();
        const requestCertificates = jest.fn(async () => {
            certificatesListener?.('server-identity', [{ serialNumber: 'abc123' }]);
        });
        const peerStub = {
            listenForCertificatesReceived: jest.fn((listener) => {
        ...
        authFetch.peers['https://api.example.com'] = {
            peer: peerStub,
            pendingCertificateRequests: []
        };
        const certs = await authFetch.sendCertificateRequest('https://api.example.com', requestedCertificates);
    ...
    test('sendCertificateRequest rejects and cleans up listener when requestCertificates throws', async () => {
        ...
        await expect(authFetch.sendCertificateRequest('https://api.example.com', requestedCertificates)).rejects.toThrow('request failed');
    ...
    test('sendCertificateRequest times out and cleans up listener when no certificate response arrives', async () => {
        ...
        try {
            ...
            const pending = authFetch.sendCertificateRequest('https://api.example.com', requestedCertificates);
            const timeoutAssertion = expect(pending).rejects.toThrow('sendCertificateRequest timed out after 30000ms waiting for certificate response from https://api.example.com');
